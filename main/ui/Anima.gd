...
func ABRE(alvo: Object, argumento: String) -> void: #abre janela
	...
	get_tree().create_tween().set_parallel().tween_property(alvo, "modulate", Color(1, 1, 1, 1), 
...
func FUNDO(alvo : Object) -> void:
	get_tree().create_tween().set_loops().set_parallel().tween_property(alvo, "position", Vector2(-350, -350),
...
func BUTTON_FOCUS(alvo: Object) -> void:
	get_tree().create_tween().set_parallel().tween_property(alvo, "scale", Vector2(1.005, 1.005), 
	...
	if not MouseFocused:
		...
		"position", alvo.pivot_offset + alvo.global_position, 
		0.1).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT).from_current()
	
	get_tree().create_tween().set_parallel().tween_property(alvo, "modulate", Color(1, 0.95, 1, 1), 
